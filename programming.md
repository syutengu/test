...
## 前置き
皆さんお疲れ様です。  
文系出身で大学時代は主に漢文の勉強をしていて算数がまったく得意ではないSYUです。  
一回目として「プログラミング(？)のすすめ」をテーマにお送りします。  

社会生活のなかで、私たちはいろんな場面で各種の問題に遭遇し判断を求められることがあります。  
それらの問題は往々にして算数問題そのまのか、数値化によってある程度理性的判断が可能になるものです。  
算数または数値化できる問題を解決するには、計算道具の力を借りるのは効率的です。  
...
本質的には四則演算レベルの簡単な算数問題で、日常で電卓の税計算機能やExcel関数、(広告まみれの)ウェブアプリ等を用いて処理されている方も多いかと思います。  
今回は道具を変えて、自作のプログラムで計算して答えを出してみませんか。  

用意必要なものは二つだけ、制御台と作業台です。  
**A制御台**とは最終的にプログラムを実行するため、コンピュータと会話(INPUTとOUTPUT)するユーザーインターフェース(場所、以下「UI」と省略)です。  
...
いつも開いているブラウザでただF12キーを押すだけで基本的な算数演算ができるほうが比較的便利だと感じます。  
...
毎回関数を呼び出して処理させるには、コンソールに関数を貼り付けてメモリに読み込ませる必要がありませんが、  
関数の内容が更新されたら、もう一度新しい内容を貼り付けてメモリに読み込ませる必要はあります。  
...
上のように、  
３番目の端数処理という引数の値を、  
無入力または`「0」`と入力の場合、端数の切り下げ処理が行われます。  
`「1」`と入力の場合、端数の切り上げ処理が行われます。  
`「45」`と入力の場合、端数の四捨五入処理が行われます。  
...
１0行目では、`「消費税額」`という関数が宣言され、`「税込金額*(適用税率/(適用税率+100))」`の計算結果の数値が代入されます。  
...
< Math { … }
    ...
    ​floor: function floor() //引数として与えた数値以下の最大の整数を返し
...
どの税率でも端数処理を適用したいので、30行目の前に「//」をつけてコメントアウトして、※  
...
また、必要されあれば税抜金額の同時出力や消費税と地方消費税の区分等、さまざまな新しい機能を後からこの関数に追加できます。  

いかがでしょうか。この課題を例にプログラミングの基礎的知識をほんの少し紹介しました。  
Excel関数等と比べるとよりきめ細かいチューニングができて、意のままのデータ自動処理装置は理論上可能だと思いませんでしょうか。  
...
## HELLO WORLD!
今回のテーマに戻りますが、  
プログラミングもコンピュータもあくまでも問題解決するための数多くある道具の中の一つに過ぎません。  
皆さんに本当におすすめしたいのはプログラミングやコンピュータという道具ではなく、  
物事を数値的・論理的に考える習慣そのものです。  

ゼロからプログラミングを勉強する、ことに独学の場合(少なくとも私のような算数得意でない人にとっては)コストは決して低いとは言えません。※  
